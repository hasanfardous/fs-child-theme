...
jQuery(document).ready(function () {
    ...
    */
    ...
    /**
     * Purchasing Script for MS Office - Checkout
     */
    jQuery('body').on('click', '#la-black-friday-deal-checkout-btn', function (event) {
        ...
        jQuery.ajax({
            ...
            success: function (result) {
                jQuery('.la-cart-special-offers-overlay-layer p').text('Added the Deal!');
                jQuery(document.body).trigger('wc_fragment_refresh');
                jQuery(document.body).trigger('added_to_cart');
                setTimeout(function () {
                    jQuery('body').trigger('update_checkout');
                    jQuery('.la-black-friday-deal-checkout').fadeOut();
                }, 2000);
                console.log(result);
            },
            error: function (xhr, ajaxOptions, thrownError) {
    ...
    // Office 2021 Purchasing Script
    jQuery('body').on('click', '.la-office-2021-purchase-wrap a.purchase-office-2021', function (event) {
        event.preventDefault();
        let product_id = 387181;
        let product_var_id = jQuery(this).data('course-var-id');
        let quantity = 1;
        console.log(product_id, product_var_id);
        jQuery.ajax({
            url: wc_add_to_cart_params.ajax_url,
            type: 'POST',
            data: {
    ...
    // Education & Training courses radio selector script
    let courseRadioEl = jQuery('.csh-course-fee form.sp-single-course-price-form .radio-container');
    let courseBtnWrapEl = jQuery('.csh-course-fee form.sp-single-course-price-form .right-sp-course-btn');
    let firstVarID = courseRadioEl.first().find('input[type=radio]').val();
    console.log('this is ', firstVarID);
    courseBtnWrapEl.find('button').val(firstVarID);
    courseRadioEl.on('click', function (event) {
        event.preventDefault();
        console.log('clicked item');
        courseRadioEl.each(function (index, item) {
            jQuery(item).removeClass('active');
            jQuery(item).find('input[type=radio]').prop('checked', false);
        });
        jQuery(this).closest('.radio-container').addClass('active');
        jQuery(this).find('input[type=radio]').prop('checked', true);
        let regularCourseID = jQuery(this).find('input[type=radio]').val();
        let lifeTimeCourseID = jQuery(this).data('lifetime-course-id');
        if (jQuery(this).find('input[type=radio]').val() == 'lifetime-access') {
            courseBtnWrapEl.find('button').val(lifeTimeCourseID);
        } else {
            courseBtnWrapEl.find('button').val(regularCourseID);
        }
    });

});

function getBlackFridayRemainingCountdown($shortform = null) {
    let blackFridayCountdownHolder = document.getElementsByClassName('la-show-black-friday-countdown-reminder')
    let blackFridayDay = new Date("2024-02-01T00:00:00Z").getTime();
    ...
    var x = setInterval(function () {
        ...
        // Print the countdown
        for (let i = 0; i < blackFridayCountdownHolder.length; i++) {
            blackFridayCountdownHolder[i].style.display = 'block';
            if ($shortform == 'yes') {
                blackFridayCountdownHolder[i].innerHTML = '<i class="fa fa-clock-o"></i> Only today left at this price!';
            } else {
                blackFridayCountdownHolder[i].innerHTML = '<i class="fa fa-clock-o"></i> Only ' + days + ' days ' + hours + ' hours ' + minutes + ' minutes ' + seconds + ' seconds left at this price!';
            }
        }
        ...
        if (timeDistance < 0) {
            clearInterval(x);
            // for (let j = 0; j < blackFridayCountdownHolder.length; j++) {
            //     blackFridayCountdownHolder[j].style.display = 'block';
            //     blackFridayCountdownHolder[j].innerHTML = "EXPIRED";
            // }
        }
    }, 1000);
}